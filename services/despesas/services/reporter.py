...
borda_tracejada = Border(left=Side(style='dashed'), right=Side(style='dashed'), top=Side(style='dashed', color="000000"), bottom=Side(style='dashed', color="000000"))
...
def criar_layout(ws):
    ...
    for i in [6,7,8,12,13,14,16]:
        ...
        ws[f'A{i}'].border = borda_tracejada
...
def escrever_mes(ws, mes, col):
    ...
    for i in [6,7,8,12,13,14,16]:
        ...
        if i in (12,16,17,18,19):
            ...
            ws[f'{col}{i}'].border = borda_tracejada
            ws[f'{col_prox}{i}'].fill = cinza_claro
            ws[f'{col_prox}{i}'].border = borda_tracejada
...
def preencher_dados(ws, dados, col):
    ...
    ws[f'{col}8'] = locale.currency(dados['inss'], grouping=True)
    ws[f'{col}10'] = dados['qtde_func']
    ws[f'{col}12'] = 'Qtde'
    ws[f'{col_prox}12'] = 'Valor'
    ...
    ws[f'{col}20'] = locale.currency(dados['valor_convenio'], grouping=True)
...
def gerar_relatorio(dados, mes, aba_nome="TOTAL GERAL"):
